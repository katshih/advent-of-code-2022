    ...
    def getNumber(self, tribe):
        if(self.number is not None):
            ...
            if(self.jobType == '+'):
                ...
                self.number = sympy.solveset(tribe[self.parents[0]].getNumber(tribe) - tribe[self.parents[1]].getNumber(tribe), 'h')
        return self.number
...
for l in inputDat:
    ...
    if(name == 'humn'):
        tribe[name] = Monkey('number', number = sympy.symbols("h"))
    elif(name == 'root'):
        tribe[name] = Monkey('equality', parents = [splitString[1], splitString[3]])
