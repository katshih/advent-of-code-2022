...
totalSize = allDirs["/"].size
unusedSize = diskSpace - totalSize
deleteSize = sizeRequired - unusedSize
print(deleteSize)

bestSize = totalSize
for key in allDirs:
    if((allDirs[key].size > deleteSize) & (allDirs[key].size < bestSize)):
        bestSize = allDirs[key].size
print(bestSize)
